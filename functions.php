<?php
/**
 * factory-neman functions and definitions
 *
 ...
 */
function factory_neman_setup() {
	/*
		* Make theme available for translation.
		* Translations can be filed in the /languages/ directory.
		* If you're building a theme based on factory-neman, use a find and replace
		* to change 'factory-neman' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'factory-neman', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'factory-neman' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'factory_neman_custom_background_args',
			array(
...
add_action( 'after_setup_theme', 'factory_neman_setup' );
 ...
 */
function factory_neman_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'factory_neman_content_width', 640 );
}
add_action( 'after_setup_theme', 'factory_neman_content_width', 0 );
 ...
 */
function factory_neman_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'factory-neman' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'factory-neman' ),
...
add_action( 'widgets_init', 'factory_neman_widgets_init' );

/**
 * Enqueue scripts and styles.
 */

add_action( 'wp_enqueue_scripts', 'factory_neman_scripts' );
remove_action('wp_head', 'print_emoji_detection_script', 7);
remove_action('wp_print_styles', 'print_emoji_styles');

add_theme_support( 'post-thumbnails', array( 'post' ) );

function factory_neman_scripts() {
	wp_enqueue_style( 'factory-neman-style', get_stylesheet_uri(), array(), _S_VERSION );
...
function create_post_sharers(){

    register_post_type('sharers',
...
add_action( 'init', 'create_post_sharers' );

function create_post_society(){
...
add_action( 'init', 'create_post_society' );


function create_post_stocks(){
...
add_action( 'init', 'create_post_stocks' );
...
function filter_function(){
	...
	if ( $query->have_posts()) :
		...
		else : $response = get_template_part('./template-parts/filter-non');
	endif;
	echo $response;
	exit;
}
